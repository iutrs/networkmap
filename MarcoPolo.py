...
class NetworkDeviceBuilder(object):
    ...
    def buildNetwordDevicesFromCDPInformation(cdp_result):
        """
        Builds an array of NetworkDevice objects from the CDP neighbors \
        detail information of a Hewlett-Packard\xc2 switch.
        :param cdp_result: The result of the command "show cdp neighbors \
            detail"
        ...
        try:
            ...
            # Not forgetting the last device since it's not followed
            # by a dotted line ('-----')
            devices.append(currentDevice)
    ...
    def extractLocalPortsFromLLDPRemoteInformation(lldp_result):
        """
        Builds an array containing the connected LLDP ports of a \
            Hewlett-Packard\xc2 switch.
        :param lldp_result: The result of the command "show lldp info \
            remote-device"
        :type lldp_result: string
        :return: An array of the of the connected ports built from the \
            provided result
        :rtype: str[]
        """
        local_ports = []
            ...
            for rawline in lldp_result.splitlines():
                if '|' in rawline:
                    ...
                    if port != 'LocalPort' and \
                        NetworkDeviceBuilder._extractSystemNameFromLine(
                            rawline) is not None:
...
class NetworkExplorer(object):
    ...
    def _get_lldp_open_ports(self, origin):
        ...
        return NetworkDeviceBuilder.extractLocalPortsFromLLDPRemoteInformation(
            remote_devices_list)
    ...
    def _send_ssh_command(self, command, prepare):
        ...
        try:
            # These 'time.sleep()' are used to give enough time for
